...
export function BaileysPanel() {
  ...
  const refreshQr = async () => { setNotice("Membuat QR baru..."); const response = await fetch("/api/admin/baileys/refresh", { method: "POST" }); const data = await response.json(); if (!data.success) setNotice(data.error); else { setStatus({ status: "connecting", qrDataUrl: null }); setNotice("QR baru sedang dibuat..."); setTimeout(load, 1200) } }
  ...
  const disconnect = async () => { setNotice(""); const response = await fetch("/api/admin/baileys/disconnect", { method: "POST" }); const data = await response.json(); setStatus(data.success ? { status: "stopped", qrDataUrl: null } : status); setNotice(data.success ? "Baileys berhasil diputuskan." : data.error) }
  return <Card className="border-teal-200 bg-white shadow-sm"><CardHeader><CardTitle className="text-teal-800">WhatsApp Testing (Baileys)</CardTitle></CardHeader><CardContent className="space-y-4"><p className="text-sm text-slate-600">Khusus testing. Jalankan worker pada server persisten dan scan QR dari perangkat admin.</p><div className="flex flex-wrap gap-2"><Button onClick={refreshQr}>Mulai / Refresh QR</Button><Button variant="outline" onClick={disconnect} disabled={!status?.status || status.status === "stopped"}><LogOut className="mr-2 h-4 w-4" />Disconnect Baileys</Button></div>{status?.qrDataUrl && <img src={status.qrDataUrl} alt="QR code WhatsApp Baileys" className="h-64 w-64 rounded border p-2" />}{status?.status && <p className="text-sm">Status: <strong>{status.status}</strong></p>}<div className="grid gap-2 sm:grid-cols-2"><Input placeholder="Nomor WhatsApp, contoh 62812..." value={to} onChange={(e) => setTo(e.target.value)} /><Input placeholder="Pesan" value={message} onChange={(e) => setMessage(e.target.value)} /></div><Button variant="outline" onClick={send} disabled={!to || !message}>Kirim pesan tes</Button>{notice && <p className="text-sm text-slate-700">{notice}</p>}</CardContent></Card>
}
