...
interface Transaction { order_id: string; user_id: string; game_name: string; package_diamonds: number; amount: number; status: string; payment_method?: string; created_at: string }
...
export function TransactionDrawer() {
  ...
  async function search(event: FormEvent) {
    event.preventDefault(); if (!transactionId.trim()) return
    setLoading(true); setSearched(true)
    try { const response = await fetch(`/api/transactions/history?transactionId=${encodeURIComponent(transactionId.trim())}`); const data = await response.json(); setTransactions(data.transactions ?? []) } finally { setLoading(false) }
  }
  return <>
