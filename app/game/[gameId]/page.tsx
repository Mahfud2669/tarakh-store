...
export default function GameDetailPage({ params }: GameDetailPageProps) {
  ...
  useEffect(() => {
    const fetchGameDetails = async () => {
      try {
        ...
        const foundGame = (gameData.games || []).find((g: Game) => g.game_id === params.gameId)
        ...
        // Fetch packages
        const packagesResponse = await fetch(`/api/games/${params.gameId}/packages`)
  ...
  const handlePayment = async () => {
    ...
    try {
      const response = await fetch('/api/payment', {
        ...
        body: JSON.stringify({
          game: game?.name,
          package: { ...selectedPackage, game_id: params.gameId },
          userId: userId.trim(),
  ...
  const isFormValid = selectedPackage && userId.trim() && (!['ml'].includes(params.gameId) || serverId.trim())
